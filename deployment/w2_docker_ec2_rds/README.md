...
### 5.a. Run Locally - Create Dockerfile and Push Image to Docker Hub

Theory: When running app and database containers using Docker Compose, the app may start before the database is ready, causing connection errors. To avoid this, containers can be started sequentially or use a “wait-for-database” script. In this case, we use the sequential approach: the database is created first, and then the app starts, expecting an existing database to connect to.

```bash
# Clone the repository
git clone https://github.com/omarleon/library-management.git
...
# Create a docker network to connect app container and db container
...
# Verify App image name
docker images

# Run container
docker run -d -p 8000:8000 --network backend_net -e DATABASE_URL="mysql+mysqlconnector://root:12345678@mysql:3306/mydb" library-management-api

# Test App and DB are working well using Endpoints and CURLs (Section 6)

# Tag, Login and Push image to Docker Hub
docker tag library-management-api omarleon/library-management:latest
docker login
docker push omarleon/library-management:latest

```

### 5.b. EC2/Docker + RDS (AWS Console)
...
```init
    ...
    - Enable automated backup: Unchecked
